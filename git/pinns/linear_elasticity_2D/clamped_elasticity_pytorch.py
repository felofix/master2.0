...
def manu_elasticity_solve(nx, ny, n_hid, n_neu, epochs, lr, activation_function = nn.Tanh(), verbose=False, random_amount = 5, k = 1):
	"""
	PARAMETERS:
...
def loss_function(x, y, net, left, right, bottom, top, internal, f_x, f_y, epoch, k):
	...
	# Neumann loss. 
	# Leftside. 
	t_x_bottom_pred = s_xx[bottom]*bottom_x + s_xy[bottom]*bottom_y
	t_y_bottom_pred = s_xy[bottom]*bottom_x + s_yy[bottom]*bottom_y
	t_bottom_x_l = t_x_bottom_pred - t_x_bottom
	t_bottom_y_l = t_y_bottom_pred - t_y_bottom

	# Top.
	t_x_top_pred = s_xx[top]*top_x + s_xy[top]*top_y
	t_y_top_pred = s_xy[top]*top_x + s_yy[top]*top_y
